...
export class CocapnSidebar implements vscode.WebviewViewProvider {
  public static readonly viewType = 'cocapn.chatView';
  private _view?: vscode.WebviewView;
  private _serverUrl: string;
  private _extensionUri: vscode.Uri;
  private _messages: { role: string; text: string }[] = [];
  ...
  public async sendWithContext(prompt: string, contextText: string, fileName: string): Promise<void> {
    this.show();
  ...
  private async _handleChat(text: string): Promise<void> {
    ...
    try {
      ...
      if (contentType.includes('text/event-stream') || contentType.includes('application/x-ndjson')) {
        ...
        const data = await resp.json() as { reply?: string; text?: string; message?: string };
