...
export interface ABTestTask {
  ...
  /** Optional: complexity level for analysis */
  complexity?: 'simple' | 'medium' | 'complex';
}

/**
 * Result of running a single task with one approach
 */
export interface ABTestResult {
  ...
  duration: number;
  /** Whether manual fixes were needed */
...
/**
 * Summary of comparing single vs tree for a single task
 */
export interface ABTestSummary {
  ...
  description: string;
  /** Single-path result */
  singleResult: ABTestResult;
  /** Tree search result */
  treeResult: ABTestResult;
  /** Which approach won */
  winner: 'single' | 'tree' | 'tie';
  /** Metrics where single was better */
  singleAdvantage: { metric: string; value: number; description: string }[];
  /** Metrics where tree was better */
  treeAdvantage: { metric: string; value: number; description: string }[];
  /** Overall score difference (tree - single) */
  scoreDifference: number;
}

/**
 * Aggregated statistics across all A/B tests
 */
export interface ABTestAggregate {
  /** Total number of tasks tested */
  totalTasks: number;
  /** Number of tasks where single won */
  singleWins: number;
  /** Number of tasks where tree won */
  treeWins: number;
  /** Number of ties */
  ties: number;
  /** Win rate for single (0-1) */
  singleWinRate: number;
  /** Win rate for tree (0-1) */
  treeWinRate: number;
  /** Tie rate (0-1) */
  tieRate: number;
  /** Average tokens used by single */
  avgSingleTokens: number;
  /** Average tokens used by tree */
  avgTreeTokens: number;
  /** Percentage difference in tokens (negative = tree uses more) */
  avgTokenDifference: number;
  /** Average duration for single (ms) */
  avgSingleDuration: number;
  /** Average duration for tree (ms) */
  avgTreeDuration: number;
  /** Percentage difference in duration (negative = tree slower) */
  avgDurationDifference: number;
  /** Average test pass rate for single */
  avgSingleTestPassRate: number;
  /** Average test pass rate for tree */
  avgTreeTestPassRate: number;
  /** Percentage difference in test pass rate (positive = tree better) */
  avgTestPassRateDifference: number;
  /** Average code quality for single */
  avgSingleCodeQuality: number;
  /** Average code quality for tree */
  avgTreeCodeQuality: number;
  /** Percentage difference in code quality (positive = tree better) */
  avgCodeQualityDifference: number;
  /** Success rate for single (0-1) */
  singleSuccessRate: number;
  /** Success rate for tree (0-1) */
...
/**
 * Options for ABTestRunner
 */
export interface ABTestOptions {
  /** Brain instance for storing results (optional) */
  brain?: Brain;
  /** Whether to store results in Brain */
  storeResults?: boolean;
  /** Timeout for single-path execution (ms) */
  singleTimeout?: number;
  /** Timeout for tree search execution (ms) */
  treeTimeout?: number;
}
...
export class ABTestRunner {
  ...
  private mockSingleResult?: ABTestResult;
  private mockTreeResult?: ABTestResult;
   ...
   */
  async runSingle(task: ABTestTask): Promise<ABTestResult> {
    ...
    try {
      ...
      const result: TreeNodeResult = await executor.executeApproach(
        task.description,
        approach,
        task.repoRoot
      );
   ...
   * @returns Promise resolving to execution result
   */
  async runTree(task: ABTestTask): Promise<ABTestResult> {
    // If mock result is set, use it (for testing)
   ...
   */
  async compare(task: ABTestTask): Promise<ABTestSummary> {
    // Run both approaches
    ...
    const singleAdvantage: { metric: string; value: number; description: string }[] = [];
    const treeAdvantage: { metric: string; value: number; description: string }[] = [];
   ...
   */
  async runBatch(tasks: ABTestTask[]): Promise<ABTestSummary[]> {
    ...
    for (const task of tasks) {
      try {
        ...
        const failureResult: ABTestResult = {
          taskId: task.id,
          approach: 'single',
          success: false,
          testPassRate: 0,
   ...
   */
  aggregate(results: ABTestSummary[]): ABTestAggregate {
    ...
    for (const result of results) {
      if (result.winner === 'single') singleWins++;
      else if (result.winner === 'tree') treeWins++;
      else ties++;
    }
   ...
   */
  generateReport(aggregate: ABTestAggregate): string {
    const lines: string[] = [];
   ...
   */
  private async storeResult(taskId: string, result: ABTestResult): Promise<void> {
   ...
   */
  setMockSingleResult(result: ABTestResult): void {
   ...
   * @param result - Mock result to return
   */
  setMockTreeResult(result: ABTestResult): void {
 ...
 */
export function createABTestTask(
  description: string,
  repoRoot: string,
  files: string[],
  expectedOutcome?: string,
...
export function createMockResult(
  ...
  tokens: number,
  duration: number,
  testPassRate: number = 1.0,
  codeQualityScore: number = 0.8
): ABTestResult {
