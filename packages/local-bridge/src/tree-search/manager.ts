...
export class ExperimentManager {
   ...
   */
  createRoot(task: string, approach: string = 'Solve directly'): TreeNode {
   ...
   */
  branchApproaches(parentId: string, approaches: string[]): TreeNode[] {
   ...
   */
  getNextToExplore(): TreeNode | null {
    const pendingNodes = Array.from(this.tree.values()).filter(
      (n) => n.status === 'pending'
    );
   ...
   */
  evaluate(nodeId: string, result: TreeNodeResult): void {
  ...
  prune(): string[] {
    ...
    const belowThreshold = Array.from(this.tree.values()).filter(
      (n) => n.score !== undefined && n.score < threshold && n.status === 'completed'
    );
   ...
   */
  getBestResult(): TreeNode | null {
    const completedNodes = Array.from(this.tree.values()).filter(
      (n) => n.status === 'completed' && n.score !== undefined
    );
   ...
   */
  getNodesByStatus(status: TreeNodeStatus): TreeNode[] {
   ...
   */
  private getParentScore(node: TreeNode): number {
    if (!node.parentId) {
      return 0;
    }
   ...
   */
  private getPendingDescendants(nodeId: string): string[] {
