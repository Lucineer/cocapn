...
import type { RepoProfile } from "./detector.js";
...
const TEMPLATES: Record<
  string,
  {
    ...
    displayName: string;
    description: string;
    match: (profile: RepoProfile) => number;
    modules: string[];
    personality: string;
  }
> = {
  "cloud-worker": {
    ...
    match: (profile) => {
      let score = 0;
      if (profile.language === "typescript") score += 0.4;
      if (profile.framework === "hono") score += 0.5;
  ...
  "web-app": {
    ...
    match: (profile) => {
      ...
      if (profile.framework === "react" || profile.framework === "vue")
        score += 0.5;
      if (profile.hasTests) score += 0.1;
      if (profile.hasCI) score += 0.1;
      return score;
    },
  ...
  "python": {
    ...
    match: (profile) => {
      let score = 0;
      if (profile.language === "python") score += 0.6;
      if (
  ...
  "dmlog": {
    ...
    match: (profile) => {
      let score = 0;
      if (profile.language === "typescript") score += 0.2;
      if (profile.framework === "react") score += 0.1;
  ...
  "makerlog": {
    ...
    match: (profile) => {
      let score = 0;
      if (profile.language === "typescript") score += 0.3;
      if (profile.language === "javascript") score += 0.2;
...
export class TemplateMatcher {
   ...
   */
  async match(profile: RepoProfile): Promise<TemplateMatch> {
    const scores: Array<{ template: string; confidence: number }> = [];
