...
const SENSITIVE_LINE_RE =
  /\b(?:password|secret|token|api[_-]?key|private[\s_-]+key|auth[\s_-]*key|access[\s_-]*key|bearer|credential)\b/i;
...
const INTERNAL_URL_RE =
  /\bhttps?:\/\/(?:localhost|127(?:\.\d+){3}|0\.0\.0\.0|192\.168(?:\.\d+){2}|10(?:\.\d+){3}|172\.(?:1[6-9]|2\d|3[01])(?:\.\d+){2}|[a-z0-9-]+\.(?:local|internal|lan))\b(?:[^\s,;'")\`\]]*)?/gi;
...
export class Sanitizer {
  ...
  sanitizeWikiPage(content: string): string {
    ...
    out = out
      .split("\n")
      .map((line) =>
        SENSITIVE_LINE_RE.test(line) ? "[redacted]" : line
      )
      .join("\n");
