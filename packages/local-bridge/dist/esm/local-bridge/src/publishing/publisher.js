...
export class Publisher {
    ...
    // Event handler
    // ---------------------------------------------------------------------------
    async onPostCommit(message, files) {
        // Skip our own commits to avoid re-triggering.
        if (message.startsWith(PUBLISHER_COMMIT_PREFIX))
            return;
        // Skip concurrent runs.
        if (this.busy)
            return;
        // Only act when wiki pages or task files are involved.
        const wikiFiles = files.filter((f) => f.startsWith("cocapn/wiki/") && f.endsWith(".md"));
        const taskFiles = files.filter((f) => f.startsWith("cocapn/tasks/") && f.endsWith(".md"));
        if (wikiFiles.length === 0 && taskFiles.length === 0)
            return;
        this.busy = true;
        try {
     ...
     */
    async publish(changedFiles = []) {
        // Separate wiki pages from task files
        const changedWikiFiles = changedFiles.filter((f) => f.startsWith("cocapn/wiki/") && f.endsWith(".md"));
     ...
     */
    renderTemplate(date, streakCount, tags, digest) {
        let tmpl;
        try {
        ...
        tmpl = tmpl.replace(/\{\{#accomplishments\}\}([\s\S]*?)\{\{\/accomplishments\}\}/g, (_match, inner) => digest.accomplishments.length === 0
            ? ""
    ...
    countStreak() {
        ...
        let existing;
        try {
            existing = new Set(readdirSync(updatesDir)
    ...
    upsertIndex(entry) {
        ...
        let index = { entries: [], updatedAt: new Date().toISOString() };
...
function parseTaskFile(filename, content) {
    ...
    const status = statusMatch?.[1] === "done" ? "done" : "active";
