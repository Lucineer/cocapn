 ...
 */
export async function authenticateConnection(ws, req, options) {
    let githubLogin;
    let githubToken;
 ...
 */
export async function validateGithubPat(token) {
    try {
 ...
 */
export function verifyPeerAuth(req, skipAuth, fleetKey) {
    if (skipAuth)
        return true;
    if (!fleetKey)
        return false;
