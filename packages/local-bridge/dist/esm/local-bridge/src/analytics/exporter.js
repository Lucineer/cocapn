...
export class AnalyticsExporter {
     ...
     */
    exportJSON(options = { format: "json" }) {
     ...
     */
    exportReportCSV(options = { format: "csv" }) {
        const period = options.period ?? this.getDefaultPeriod();
        const report = this.collector.aggregate(period);
        const lines = [];
        // Report metadata
     ...
     */
    exportMarkdown(options = { format: "markdown" }) {
        const period = options.period ?? this.getDefaultPeriod();
        const report = this.collector.aggregate(period);
        const lines = [];
        // Title
