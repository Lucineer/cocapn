...
import { HealthChecker, checkGit, checkBrain, checkDisk, checkWebSocket } from "../health/index.js";
import { OfflineQueue } from "../cloud-bridge/offline-queue.js";
...
import { handleStreamingDiffStart, handleStreamingDiffChunk, handleStreamingDiffStatus, handleStreamingDiffFinalize, handleStreamingDiffRollback, } from "../handlers/streaming-diff.js";
import { handleCloudStatus, handleCloudSubmitTask, handleCloudTaskResult, } from "../handlers/cloud.js";
...
export class BridgeServer extends EventEmitter {
    ...
    setupHealthChecks(repoGraph) {
        ...
        this.healthChecker.addCheck('cloud', async () => {
            const cloudConnector = this.options.bridge?.cloudConnector;
            if (!cloudConnector) {
     ...
     */
    async handleTenantHttpRequest(req, res, url) {
        ...
        try {
            ...
            const tenantId = segments[0] || req.headers["x-tenant-id"];
            ...
            if (segments.length === 1 && method === "PUT") {
                ...
                const updated = await registry.updateTenant(tenantId, updates);
            ...
            if (segments.length === 1 && method === "DELETE") {
                ...
                if (tBridge)
                    tBridge.disposeContext(tenantId);
                setJson(200, { ok: true });
                return;
            }
    ...
    // Authentication
    // ---------------------------------------------------------------------------
    async authenticateAndConnect(ws, req, clientId) {
