...
export class BackpressureManager {
     ...
     */
    getQueueHealth(totalItems, runningItems) {
     ...
     */
    shouldAccept(priority, totalItems) {
    ...
    onRateLimit(provider) {
        ...
        const maxConcurrent = Math.max(1, Math.floor(this.config.maxConcurrency / Math.pow(2, current + 1)));
        this.providerPenalty.set(provider, current + 1);
        this.providerCooldowns.set(provider, Date.now() + 60_000); // 1 min cooldown
        console.warn(`[queue] Backpressure: ${provider} rate-limited, ` +
     ...
     */
    getEffectiveConcurrency(provider) {
        const penalty = this.providerPenalty.get(provider) ?? 0;
        if (penalty === 0)
