...
export class RequestQueue {
    config;
    queue = [];
    running = new Set(); // item IDs currently executing
    items = new Map(); // all known items
    deadLetterQueue = [];
    waiters = new Map();
    rateLimiter;
    backpressure;
    ...
    async enqueue(item) {
        ...
        if (!this.backpressure.shouldAccept(item.priority, totalItems)) {
            throw new Error(`Queue rejecting request: backpressure active ` +
     ...
     * Throws on timeout.
     */
    async waitForResult(itemId, timeout) {
    ...
    insertByPriority(item) {
        ...
        const idx = this.queue.findIndex((existing) => existing.priority < item.priority ||
    ...
    completeItem(item, result, error) {
