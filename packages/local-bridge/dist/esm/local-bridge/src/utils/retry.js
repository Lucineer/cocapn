 ...
 */
export async function withRetry(fn, options = {}, onRetry) {
    const { maxAttempts = 3, baseDelay = 1000, maxDelay = 30000, backoffFactor = 2, jitter = true, isRetryable = () => true, } = options;
    const errors = [];
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (attempt === maxAttempts) {
                const retryError = new Error(`Operation failed after ${maxAttempts} attempts: ${err.message}`);
...
export function createRetryable(fn, defaultOptions = {}) {
    const retryableFn = (async (...args) => {
        return withRetry(() => fn(...args), defaultOptions);
    });
    retryableFn.withRetry = (options) => {
 ...
 */
export function isRetryableHttpCode(status) {
