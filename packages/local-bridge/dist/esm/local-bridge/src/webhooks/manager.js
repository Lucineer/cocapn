 ...
 */
function getWebhooksStoragePath(repoRoot) {
...
export class WebhookManager {
     ...
     */
    async triggerEvent(type, payload) {
        const event = {
            id: this.generateId(),
            type,
            payload,
        ...
        const matchingWebhooks = Array.from(this.webhooks.values()).filter((w) => w.enabled && w.events.includes(type));
    ...
    async retryFailed(maxRetries = 3) {
        const failedDeliveries = Array.from(this.deliveries.values()).filter((d) => d.status === 'failed' && d.attempts < maxRetries);
     ...
     */
    clearOldDeliveries(olderThanMs = 7 * 24 * 60 * 60 * 1000) {
