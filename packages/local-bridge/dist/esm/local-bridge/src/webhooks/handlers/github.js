...
export class GitHubWebhookHandler {
     ...
     */
    async handlePayload(eventType, payload) {
        const cocapnEvents = [];
        try {
            switch (eventType) {
                case 'push':
                    const pushEvents = this.handlePush(payload);
                    cocapnEvents.push(...pushEvents);
                    break;
                case 'pull_request':
                    const prEvents = this.handlePullRequest(payload);
                    cocapnEvents.push(...prEvents);
                    break;
                case 'issues':
                    const issueEvents = this.handleIssues(payload);
     ...
     */
    static verifySignature256(payload, signature, secret) {
        const crypto = require('crypto');
