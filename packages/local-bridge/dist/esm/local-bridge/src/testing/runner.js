...
export class TestRunner {
     ...
     */
    parseVitestOutput(stdout, stderr, testFile) {
        try {
     ...
     */
    parseAllTestsOutput(stdout, stderr) {
        try {
            // Try to parse JSON output
            const jsonMatch = stdout.match(/\{[\s\S]*\}/);
            if (jsonMatch) {
     ...
     */
    extractTestResult(data, testFile) {
        const failures = [];
        let passed = 0;
        let failed = 0;
     ...
     */
    extractAllTestsResult(data) {
        const files = [];
        let totalPassed = 0;
        let totalFailed = 0;
        if (data.testResults && Array.isArray(data.testResults)) {
            for (const fileResult of data.testResults) {
                const filePassed = (fileResult.assertionResults || []).filter((r) => r.status === "passed").length;
                const fileFailed = (fileResult.assertionResults || []).filter((r) => r.status === "failed").length;
                const failures = [];
     ...
     */
    parseTextOutput(stdout, testFile) {
