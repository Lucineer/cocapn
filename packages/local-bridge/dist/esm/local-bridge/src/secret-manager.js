...
export class SecretManager {
    ...
    async rotate() {
        ...
        if (!this.identity)
            throw new Error("COCAPN-031: Cannot rotate: no current identity loaded - Load your identity first with: cocapn-bridge secret load");
     ...
     */
    async storeIdentityInKeychain(identity) {
    ...
    async encrypt(plaintext) {
        const recipient = this.recipient ?? this.readRecipientsFile();
        if (!recipient)
            throw new Error("COCAPN-032: No public key - Initialize age encryption to encrypt secrets for fleet members: cocapn-bridge secret init");
        const age = await this.loadAgeModule();
        const encrypter = new age.Encrypter();
        encrypter.addRecipient(recipient);
    ...
    async decrypt(ciphertext) {
        if (!this.identity)
            throw new Error("COCAPN-033: No identity loaded - Load your age identity with: cocapn-bridge secret load");
        const age = await this.loadAgeModule();
    ...
    async decryptFileIntoCache(filePath) {
        if (!this.identity)
            return;
        try {
            const age = await this.loadAgeModule();
    ...
    async deriveRecipient(identity) {
        try {
            const age = await this.loadAgeModule();
    ...
    updateConfigPublicKey(recipient) {
