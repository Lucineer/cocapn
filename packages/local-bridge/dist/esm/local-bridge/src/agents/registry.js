...
export class AgentRegistry {
    ...
    resolveHybrid(agentId) {
        ...
        if (this.cloudRegistry?.get(agentId)) {
            const stub = this.makeCloudStub(agentId, this.cloudRegistry.get(agentId).getWorkerUrl());
     ...
     */
    resolveWithRunningCheck(agentId, isRunning) {
        const resolution = this.resolveHybrid(agentId);
        if (!resolution)
            return undefined;
        const localIsRunning = resolution.source === "local" && isRunning(agentId);
     ...
     */
    loadFromPrivateRepo(repoRoot, soulRelPath = "soul.md") {
    ...
    loadFromPublicConfig(repoRoot) {
        ...
        const config = raw;
        if (!Array.isArray(config.agents?.available))
            return;
    ...
    loadAgentFiles(repoRoot) {
        ...
        for (const filename of entries) {
            if (!filename.endsWith(".agent.yml"))
                continue;
    ...
    parseAgentDefinition(raw, filename) {
        ...
        return {
            ...
            args: Array.isArray(def["args"])
                ? def["args"].filter((a) => typeof a === "string")
                : [],
            env: this.parseEnvObject(def["env"]),
            capabilities: Array.isArray(def["capabilities"])
                ? def["capabilities"].filter((c) => typeof c === "string")
                : [],
