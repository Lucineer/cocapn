...
export class HandoffProcessor {
     ...
     */
    async process(initialResult, routeToModule, state) {
        ...
        while ((currentResult.type === 'handoff' || currentResult.type === 'multi') && handoffState.depth < this.maxDepth) {
            ...
            if (handoffState.previousModule === handoff.module) {
                if (this.enableLogging) {
                    console.warn(`[handoff] Circular handoff detected: ${handoff.module} → ${handoff.module}`);
                }
            ...
            // Log handoff
            if (this.enableLogging) {
                console.info(`[handoff] ${handoffState.previousModule || 'initial'} → ${handoff.module} ` +
        ...
        if (currentResult.type === 'handoff' && handoffState.depth >= this.maxDepth) {
            if (this.enableLogging) {
                console.warn(`[handoff] Max depth exceeded (${this.maxDepth}), forcing response`);
            }
            return {
                type: 'response',
                content: 'I got stuck trying to complete your request. Let me try a different approach.',
     ...
     */
    static handoff(module, context, options) {
        return {
            type: 'handoff',
            handoff: {
                module,
     ...
     */
    static multi(responses, handoffs = []) {
        return {
            type: 'multi',
            responses,
            handoffs,
     ...
     */
    wouldBeCircular(targetModule, state) {
    ...
    estimateTokenCost(state) {
        ...
        const contextCost = state.chain.reduce((sum, entry) => sum + entry.context.length * 0.25, // Rough token estimate
        0);
