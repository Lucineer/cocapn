...
export class TreeSearch {
    ...
    async search(task, options) {
        ...
        while (!this.manager.isComplete()) {
            ...
            const result = await this.executor.executeApproach(task, node.approach, this.repoRoot);
            ...
            if (config.enableBranching && result.success && (node.score || 0) > config.pruningThreshold) {
                try {
                    const subResult = await this.generator.generateApproaches(task, 2, `Parent approach: ${node.approach}`);
     ...
     */
    setMockGeneratorResponse(response) {
