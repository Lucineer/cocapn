...
export class TreeSearchExecutor {
     ...
     */
    async executeApproach(task, approach, repoRoot, context) {
        ...
        try {
            ...
            const result = await execAsync(`${this.claudePath} -p "${escapedPrompt}" --model ${this.defaultModel} --max-turns ${this.maxTurns}`, {
                cwd: repoRoot,
