...
program
    ...
    .version("0.1.0")
    .option("--repo <path>", "Path to the private repo root (default: current directory)", process.cwd())
    .option("--repo-path <path>", "Alias for --repo")
    .option("--public <path>", "Path to the public repo root (default: same as --repo)")
    .option("--port <number>", "WebSocket server port (default: 8787)", (v) => parseInt(v, 10))
    .option("--tunnel", "Start a Cloudflare tunnel via cloudflared (requires cloudflared in PATH)")
    .option("--no-auth", "Disable GitHub PAT authentication (for local development only)")
    .action(async (opts) => {
...
function startTunnel(port) {
    ...
    if (!cloudflared) {
        console.error("[tunnel] cloudflared not found in PATH. Install it from https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/");
        return;
    }
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (!candidate.startsWith("/"))
                return candidate; // Assume PATH resolution works
        }
        catch {
            // ignore
        }
