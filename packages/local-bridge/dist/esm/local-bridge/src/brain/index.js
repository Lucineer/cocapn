 ...
 */
import { readFileSync, writeFileSync, existsSync, readdirSync, mkdirSync, rmdirSync, } from "fs";
import { join, extname, basename } from "path";
import { homedir } from "os";
import { InvertedIndex } from "../utils/inverted-index.js";
import { createEmbeddingProvider } from "./embedding.js";
import { createVectorStore } from "./vector-store.js";
import { createHybridSearch } from "./hybrid-search.js";
...
export class Brain {
    ...
    sync;
    wikiIndex;
    wikiIndexInitialized = false;
    hybridSearch;
    vectorStore = null;
    embeddingProvider = null;
    repoGraph = null;
    memoryManager = null;
    constructor(repoRoot, config, sync, repoGraph, memoryManagerOptions) {
    ...
    initializeVectorSearch() {
        ...
        setImmediate(async () => {
            ...
            try {
                ...
                // Create vector store
                const vectorStore = await createVectorStore(this.repoRoot, this.embeddingProvider, vectorConfig.dimensions || 384);
     ...
     */
    async addWikiPage(sourcePath, destName) {
        const release = await acquireLock();
        try {
            const wikiDir = join(this.repoRoot, "cocapn", "wiki");
            ensureDir(wikiDir);
     ...
     */
    async updateGraphFile(filePath) {
        if (this.repoGraph) {
            try {
     ...
     */
    async removeGraphFile(filePath) {
        if (this.repoGraph) {
            try {
    ...
    while (true) {
        try {
            ...
            if (Date.now() >= deadline) {
                throw new Error(`Brain lock acquisition timed out after ${LOCK_TIMEOUT}ms ` +
                    `(lock: ${lockPath})`);
            }
...
function extractExcerpt(content, lower) {
