...
export class ConversationMemory {
     ...
     */
    async extractAndStore(userMessage, agentResponse) {
        const storedKeys = [];
        ...
        if (this.options.heuristicExtraction) {
            ...
            for (const fact of facts) {
                if (fact.confidence < this.options.storeThreshold)
                    continue;
     ...
     */
    async buildMemoryPrompt(userMessage) {
     ...
     */
    async extractFactsLLM(userMessage, agentResponse) {
        if (!this.llm)
            return [];
        const prompt = [
        ...
        try {
            const response = await this.llm.chat([{ role: "user", content: prompt }], { maxTokens: 500, temperature: 0 });
     ...
     */
    scoreRelevance(key, value, message) {
        ...
        if (keyLower.includes("project"))
            return Math.max(score, 0.4);
        if (keyLower.includes("preference"))
