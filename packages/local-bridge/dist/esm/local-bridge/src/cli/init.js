...
async function runWizard(baseDir, skipBrowser) {
...
function cloneTemplate(type, destDir, login, repoName, domain, token) {
...
function replacePlaceholders(dir, login, domain) {
    const targets = [
        join(dir, "cocapn.yml"),
...
function pushRepo(dir, token, login) {
    try {
        execFileSync("git", ["-c", `user.email=${login}@users.noreply.github.com`, "-c", `user.name=${login}`, "push", "-u", "origin", "HEAD"], { cwd: dir, stdio: "ignore" });
    }
 ...
 */
function stripPatFromRemote(dir, login, repoName) {
    try {
        execFileSync("git", ["remote", "set-url", "origin", `https://github.com/${login}/${repoName}.git`], { cwd: dir, stdio: "ignore" });
    }
    catch { /* non-fatal — user can run the fix manually */ }
}
async function enablePages(token, login, repo) {
    try {
...
function startBridge(privateDir) {
    try {
        const bridgeBin = new URL("../../../dist/esm/main.js", import.meta.url).pathname;
        if (!existsSync(bridgeBin))
            return undefined;
        const child = spawn(process.execPath, [bridgeBin, "--repo", privateDir, "--no-auth"], { detached: true, stdio: "ignore" });
        child.unref();
        return child.pid;
    }
    catch {
