 ...
 */
export async function handleWebhookList(context, sender) {
 ...
 */
export async function handleWebhookCreate(context, sender, params) {
    const webhookManager = context.getModuleManager().get('webhooks');
    if (!webhookManager) {
        await sender({
            jsonrpc: '2.0',
    ...
    try {
        const webhook = webhookManager.createWebhook(params.name, params.url, params.events, params.secret);
        await sender({
            jsonrpc: '2.0',
            id: null,
            result: {
 ...
 * Deletes a webhook
 */
export async function handleWebhookDelete(context, sender, params) {
    const webhookManager = context.getModuleManager().get('webhooks');
    if (!webhookManager) {
        await sender({
            jsonrpc: '2.0',
 ...
 * Updates a webhook
 */
export async function handleWebhookUpdate(context, sender, params) {
    const webhookManager = context.getModuleManager().get('webhooks');
    if (!webhookManager) {
        await sender({
            jsonrpc: '2.0',
 ...
 */
export async function handleWebhookGetDeliveries(context, sender, params) {
    const webhookManager = context.getModuleManager().get('webhooks');
    if (!webhookManager) {
        await sender({
            jsonrpc: '2.0',
 ...
 */
export async function handleWebhookTrigger(context, sender, params) {
    const webhookManager = context.getModuleManager().get('webhooks');
    if (!webhookManager) {
        await sender({
            jsonrpc: '2.0',
 ...
 */
export async function handleWebhookRetryFailed(context, sender, params) {
    const webhookManager = context.getModuleManager().get('webhooks');
    if (!webhookManager) {
        await sender({
            jsonrpc: '2.0',
