 ...
 */
export function parseModuleInstallIntent(content) {
    const lower = content.toLowerCase().trim();
    // Explicit git URL
    const urlMatch = lower.match(/(?:install|add)\s+(?:module\s+)?(?:from\s+)?((https?:\/\/|git@)[^\s]+)/);
    if (urlMatch?.[1]) {
    ...
    const knownMatch = lower.match(/^(?:install|add)\s+(?:the\s+)?(?:module\s+)?([a-z][a-z0-9-]+)(?:\s+module)?$/);
    if (knownMatch?.[1]) {
 ...
 */
export function parsePeerQueryIntent(content) {
    const lower = content.toLowerCase().trim();
    // "ask <domain> [for] <fact-key>"
    const askMatch = lower.match(/ask\s+([a-z][a-z0-9.-]+(?:log|bridge)?(?:\.ai|\.io|:\d+)?)\s+(?:for\s+)?(.+)/);
    ...
    // "<question>? ask <domain>"
    const trailMatch = lower.match(/^(.+?)\??\s+ask\s+([a-z][a-z0-9.-]+(?:log|bridge)?)(?:\s|$)/);
 ...
 */
export function parseSkinIntent(content) {
    const lower = content.toLowerCase().trim();
