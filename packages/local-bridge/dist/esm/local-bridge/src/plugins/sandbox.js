 ...
 */
function validatePath(basePath, targetPath, label) {
...
export class PluginSandbox {
     ...
     */
    async execute(pluginPath, skillEntry, input, context) {
     ...
     */
    async executeRpc(pluginPath, skillEntry, method, params, context) {
        const startTime = Date.now();
        const skillPath = join(pluginPath, skillEntry);
        // Validate paths to prevent directory traversal and code injection
        validatePath(pluginPath, skillEntry, 'skill entry');
     ...
     */
    buildPermissionPreamble(permissions) {
     ...
     */
    static isNetworkAllowed(host, permissions) {
     ...
     */
    static isFsAccessAllowed(path, permissions, write) {
     ...
     */
    static isShellAllowed(command, permissions) {
        return permissions.some(p => {
            if (p.type !== 'shell')
                return false;
