...
function readMemories(memoryDir) {
...
function readWikiFiles(wikiDir) {
...
function loadBrainEntries(repoRoot) {
...
function loadKnowledgeEntries(repoRoot, typeFilter) {
    const paths = resolvePaths(repoRoot);
    if (!paths)
        return [];
...
function loadWikiEntries(repoRoot) {
    const paths = resolvePaths(repoRoot);
    if (!paths)
        return [];
    return readWikiFiles(paths.wikiDir);
}
function loadChatHistory(repoRoot, sessionId) {
    const paths = resolvePaths(repoRoot);
    if (!paths)
        return [];
...
// ─── Subcommand actions ─────────────────────────────────────────────────────
function brainAction(repoRoot, format, outputPath) {
    const paths = resolvePaths(repoRoot);
    if (!paths) {
...
function chatAction(repoRoot, sessionId, format, outputPath) {
    const paths = resolvePaths(repoRoot);
    if (!paths) {
        console.log(yellow("No cocapn/ directory found. Run cocapn setup to get started."));
        process.exit(1);
...
function wikiAction(repoRoot, outputPath) {
    const paths = resolvePaths(repoRoot);
    if (!paths) {
        console.log(yellow("No cocapn/ directory found. Run cocapn setup to get started."));
        process.exit(1);
...
function knowledgeAction(repoRoot, format, typeFilter, outputPath) {
    const paths = resolvePaths(repoRoot);
    if (!paths) {
        console.log(yellow("No cocapn/ directory found. Run cocapn setup to get started."));
        process.exit(1);
...
export function createExportCommand() {
    return new Command("export")
        ...
        .action((opts) => {
        brainAction(process.cwd(), opts.format, opts.output);
    }))
        ...
        chatAction(process.cwd(), sessionId, opts.format, opts.output);
    }))
        .addCommand(new Command("wiki")
        ...
        .addCommand(new Command("knowledge")
        ...
        .option("-o, --output <path>", "Write to file instead of stdout")
        .action((opts) => {
        knowledgeAction(process.cwd(), opts.format, opts.type, opts.output);
    }));
}
