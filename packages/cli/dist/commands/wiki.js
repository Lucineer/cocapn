...
export function resolveWikiDir(repoRoot) {
    ...
    if (existsSync(join(cocapnDir, "wiki")))
        return join(cocapnDir, "wiki");
    if (existsSync(join(repoRoot, "wiki")))
        return join(repoRoot, "wiki");
    return null;
}
export function ensureWikiDir(repoRoot) {
    const cocapnDir = join(repoRoot, "cocapn");
...
function slugToPath(wikiDir, slug) {
...
export function getPage(wikiDir, slug) {
...
export function searchWiki(wikiDir, query) {
    if (!existsSync(wikiDir))
        return [];
...
// ─── Subcommands ────────────────────────────────────────────────────────────
function listAction(repoRoot, json) {
...
function getAction(repoRoot, slug, json) {
    const wikiDir = resolveWikiDir(repoRoot);
    if (!wikiDir) {
        console.log(yellow("No wiki directory found. Run cocapn setup to get started."));
        process.exit(1);
...
function editAction(repoRoot, slug) {
...
function newAction(repoRoot, slug) {
    const wikiDir = ensureWikiDir(repoRoot);
    const path = slugToPath(wikiDir, slug);
    if (existsSync(path)) {
...
function deleteAction(repoRoot, slug) {
    const wikiDir = resolveWikiDir(repoRoot);
    if (!wikiDir) {
        console.log(yellow("No wiki directory found. Run cocapn setup to get started."));
        process.exit(1);
...
// ─── Command ────────────────────────────────────────────────────────────────
export function createWikiCommand() {
