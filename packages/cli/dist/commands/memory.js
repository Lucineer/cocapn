...
// ─── Core helpers ───────────────────────────────────────────────────────────
export function resolveMemoryPaths(repoRoot) {
...
export function loadAllEntries(repoRoot) {
...
export function loadEntriesByType(repoRoot, type) {
    const paths = resolveMemoryPaths(repoRoot);
    if (!paths)
        return [];
    switch (type) {
...
// ─── Subcommands ────────────────────────────────────────────────────────────
function listAction(repoRoot, type, json) {
    const paths = resolveMemoryPaths(repoRoot);
    if (!paths) {
    ...
    const result = { entries, total: entries.length, byType };
    if (json) {
...
function getAction(repoRoot, key, json) {
    const paths = resolveMemoryPaths(repoRoot);
    if (!paths) {
        console.log(yellow("No cocapn/ directory found. Run cocapn setup to get started."));
        process.exit(1);
    ...
    const matches = allEntries.filter((e) => e.key === key || e.key.endsWith(`.${key}`) || e.key === `${key}.md`);
    if (matches.length === 0) {
...
function setAction(repoRoot, key, value) {
    const paths = resolveMemoryPaths(repoRoot);
    if (!paths) {
        console.log(yellow("No cocapn/ directory found. Run cocapn setup to get started."));
        process.exit(1);
...
function deleteAction(repoRoot, key) {
    const paths = resolveMemoryPaths(repoRoot);
    if (!paths) {
        console.log(yellow("No cocapn/ directory found. Run cocapn setup to get started."));
        process.exit(1);
...
function searchAction(repoRoot, query, json) {
    const paths = resolveMemoryPaths(repoRoot);
    if (!paths) {
        console.log(yellow("No cocapn/ directory found. Run cocapn setup to get started."));
        process.exit(1);
    ...
    const allEntries = loadAllEntries(repoRoot);
    const matches = allEntries.filter((e) => e.key.toLowerCase().includes(lowerQuery) ||
