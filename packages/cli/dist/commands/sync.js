...
export function resolveRepoPaths(cwdOverride) {
    const cwd = cwdOverride ?? process.cwd();
    // Check if we're inside a brain repo (has cocapn/memory or memory/)
    const isBrain = existsSync(join(cwd, "cocapn", "memory")) ||
    ...
    if (existsSync(configPath)) {
        ...
        if (privateMatch)
            configPublic = privateMatch[1].trim();
    }
    ...
    let siblingPublic = null;
    if (isBrain) {
        try {
            const dirs = execSync("ls ../", { cwd, encoding: "utf-8" }).trim().split("\n");
            const publicDir = dirs.find((d) => d.includes(".") && existsSync(join(cwd, "..", d, "cocapn.yml")));
            if (publicDir)
                siblingPublic = join(cwd, "..", publicDir);
        }
        catch {
            // ignore
        }
...
export function parseStatusPorcelain(output) {
    ...
    return output
        ...
        if (arrow !== -1)
            return path.slice(arrow + 4).trim();
        return path;
    });
}
/** Get the repo status. */
...
function autoCommit(repoPath, message) {
    const porcelain = git(repoPath, "status --porcelain");
...
// --- Core sync logic ---
function syncRepo(repoPath, repo, commitMsg) {
...
// --- Display helpers ---
function printRepoStatus(label, status) {
    ...
    if (status.hasRemote) {
        const tracking = [];
        if (status.ahead > 0)
            tracking.push(green(`${status.ahead} ahead`));
        if (status.behind > 0)
            tracking.push(yellow(`${status.behind} behind`));
        if (tracking.length === 0)
            tracking.push(gray("up to date"));
...
function printSyncResults(results) {
    for (const r of results) {
        ...
        const parts = [];
        if (r.committed)
            parts.push(green(`committed ${r.files.length} file(s)`));
        else
            parts.push(gray("no changes"));
        if (r.pushed)
            parts.push(green("pushed"));
        else if (r.files.length > 0)
            parts.push(yellow("not pushed"));
...
export function createSyncCommand() {
    return (new Command("sync")
        ...
        .addCommand(new Command("private")
