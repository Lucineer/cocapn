...
import { existsSync, readFileSync, writeFileSync, mkdirSync, cpSync, readdirSync, rmSync, statSync, } from "fs";
...
export function createBackupDir(repoRoot, prefix) {
    const backupDir = join(repoRoot, "cocapn", "backups", `${prefix}-${timestamp()}`);
    mkdirSync(backupDir, { recursive: true });
    return backupDir;
}
function backupFile(repoRoot, backupDir, relativePath) {
...
function backupDirectory(repoRoot, backupDir, relativePath) {
    const src = join(repoRoot, relativePath);
...
// ─── Reset operations ───────────────────────────────────────────────────────
export function resetBrain(repoRoot, backupDir) {
    const backedUp = [];
    const cleared = [];
...
export function resetKnowledge(repoRoot, backupDir) {
    const backedUp = [];
    const cleared = [];
...
export function resetAll(repoRoot, backupDir) {
...
export function createResetCommand() {
    return new Command("reset")
        ...
        if (!options.force) {
            ...
            const confirmed = await confirmPrompt(`Reset ${target}? A backup will be created first.`);
            if (!confirmed) {
