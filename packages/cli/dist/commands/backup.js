...
import { existsSync, readFileSync, writeFileSync, mkdirSync, readdirSync, rmSync, statSync, createReadStream, } from "fs";
...
function backupDirPath(repoRoot) {
    return join(repoRoot, BACKUP_DIR);
}
function backupFilePath(repoRoot, name) {
    return join(repoRoot, BACKUP_DIR, `${name}.tar.gz`);
}
function manifestPath(repoRoot, name) {
    return join(repoRoot, BACKUP_DIR, `${name}.json`);
}
export function resolveCocapnDir(repoRoot) {
...
// ─── Create backup ──────────────────────────────────────────────────────────
export async function createBackup(repoRoot) {
    ...
    execSync(`cd "${repoRoot}" && tar czf "${archivePath}" ${excludeArgs} ${includeArgs}`, { stdio: "pipe" });
...
export function listBackups(repoRoot) {
    ...
    const entries = readdirSync(bDir)
        ...
        .map((f) => {
        try {
            const manifest = JSON.parse(readFileSync(join(bDir, f), "utf-8"));
...
// ─── Restore backup ─────────────────────────────────────────────────────────
export async function restoreBackup(repoRoot, backupName, preRestoreBackup) {
    ...
    execSync(`cd "${repoRoot}" && tar xzf "${archivePath}"`, { stdio: "pipe" });
...
// ─── Clean backups ──────────────────────────────────────────────────────────
export function cleanBackups(repoRoot, keep) {
...
export function createBackupCommand() {
    return new Command("backup")
        ...
        const confirmed = await confirmPrompt(`Restore from ${name}? This will overwrite current agent data.`);
        if (!confirmed) {
        ...
        .addCommand(new Command("clean")
        .description("Remove old backups")
        .option("-k, --keep <n>", "Number of backups to keep", (v) => parseInt(v, 10), 5)
