...
export function readLocalStatus(repoRoot) {
    let facts = 0;
    let memories = 0;
    let wikiPages = 0;
    ...
    if (existsSync(memoriesPath)) {
        try {
            const data = JSON.parse(readFileSync(memoriesPath, "utf-8"));
...
// ─── Terminal renderer ──────────────────────────────────────────────────────
export function renderStatus(status, offline) {
    const lines = [];
    if (offline) {
...
function renderBridgeNotRunning() {
    const lines = [
...
// ─── Watch mode ─────────────────────────────────────────────────────────────
async function watchMode(host, port, repoRoot) {
