...
function invitesDirPath(repoRoot: string): string {
  return join(repoRoot, INVITES_DIR);
}

function inviteFilePath(repoRoot: string, code: string): string {
...
export function listInvites(repoRoot: string): Invite[] {
  const dir = invitesDirPath(repoRoot);
  ...
  return readdirSync(dir)
    ...
    .map((f) => {
      try {
        return JSON.parse(readFileSync(join(dir, f), "utf-8")) as Invite;
      } catch {
        return null;
      }
    })
...
export function revokeInvite(repoRoot: string, code: string): Invite {
  ...
  const invite: Invite = JSON.parse(readFileSync(filePath, "utf-8"));
  ...
  return new Command("invite")
    ...
    .addCommand(
      new Command("accept")
        ...
        .action((code: string, options: { dir?: string }) => {
          const repoRoot = process.cwd();
