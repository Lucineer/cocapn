...
export class MCPServer {
    ...
    async dispatch(req) {
        switch (req.method) {
            ...
            default:
                throw Object.assign(new Error(`Method not found: ${req.method}`), { code: JsonRpcErrorCode.MethodNotFound });
        }
    }
    // ---------------------------------------------------------------------------
    // Handlers
    ...
    async handleResourceRead(params) {
        ...
        throw Object.assign(new Error(`Resource not found: ${params.uri}`), { code: JsonRpcErrorCode.InvalidParams });
    }
    // ---------------------------------------------------------------------------
    // Helpers
    // ---------------------------------------------------------------------------
    requireInitialized() {
        if (!this.initialized) {
            throw Object.assign(new Error("Server not initialized — client must call initialize first"), { code: JsonRpcErrorCode.InvalidRequest });
        }
    }
