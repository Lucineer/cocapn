...
const fleet_manager_js_1 = require("./fleet-manager.cjs");
// ---------------------------------------------------------------------------
// Fleet Client Class
// ---------------------------------------------------------------------------
class FleetClient {
    ...
    constructor(config, manager) {
        this.config = config;
        this.manager = manager || new fleet_manager_js_1.FleetManager();
        this.state = {
            peers: [],
            connected: false,
    ...
    async connect() {
        try {
            const result = this.manager.getRegistry().registerAgent(this.config.agentId, {
            ...
            }, this.config.desiredFleetId, this.config.preferredRole);
            // Update state
     ...
     */
    async sendProgress(taskId, progress, status, message) {
    ...
    async sendHeartbeat() {
        if (!this.state.connected)
            return;
        const status = this.state.currentTasks.size > 0 ? 'busy' : 'idle';
        // Update in registry
        this.manager.updateHeartbeat(this.config.agentId, status, this.state.currentTasks.size > 0 ? Array.from(this.state.currentTasks)[0] : undefined, this.state.currentTasks.size / 10 // Assume max 10 concurrent tasks
        );
