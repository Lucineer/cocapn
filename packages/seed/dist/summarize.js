...
export function shouldSummarize(memory) {
...
function formatSummary(s) {
    ...
    if (s.factsLearned.length)
        parts.push(`facts: ${s.factsLearned.length}`);
    if (s.unansweredQuestions.length)
        parts.push(`unanswered: ${s.unansweredQuestions.join('; ')}`);
    return parts.join(' | ');
}
