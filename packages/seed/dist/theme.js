...
export function loadTheme(dir, preset) {
    const base = { accent: '#22c55e', accent2: '#16a34a', mode: 'dark', font: 'monospace' };
    if (preset && PRESETS[preset])
        Object.assign(base, PRESETS[preset]);
    for (const name of ['theme.css', 'cocapn/theme.css']) {
        const fp = join(resolve(dir), name);
        if (!existsSync(fp))
            continue;
        const css = readFileSync(fp, 'utf-8');
        const g = (n) => (css.match(new RegExp(`--${n}:\\s*([^;\\s}]+)`)) || [])[1]?.trim();
        if (g('accent') || g('color-primary'))
            base.accent = g('accent') || g('color-primary');
        if (g('color-secondary'))
            base.accent2 = g('color-secondary');
    }
    return base;
}
