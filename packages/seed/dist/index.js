...
async function resolveApiKey(config) {
    const llm = config.llm ?? {};
    // 1. Config
    if (llm.apiKey ?? config.apiKey)
        return llm.apiKey ?? config.apiKey;
...
// ─── Command handlers ──────────────────────────────────────────────────────────
function cmdWhoami(awareness, memory) {
...
function cmdMemoryList(memory) {
    ...
    if (lines.length === 0)
        lines.push(`${GR}(empty — no memories yet)${R}`);
    return lines.join('\n');
}
function cmdMemorySearch(memory, query) {
    ...
    if (results.gitLog.length > 0) {
        ...
        for (const entry of results.gitLog)
            lines.push(`  ${GR}${entry}${R}`);
    }
    if (lines.length === 0)
        lines.push(`${GR}No matches for "${query}"${R}`);
    return lines.join('\n');
}
function cmdMemoryClear(memory) {
    memory.clear();
    return '\x1b[90mMemory cleared.\x1b[0m';
}
function cmdHelp(agentName) {
...
function cmdExport(memory) {
    const GR = '\x1b[90m', G = '\x1b[32m', R = '\x1b[0m';
    const lines = ['# Cocapn Memory Export', '', `Exported: ${new Date().toISOString()}`, ''];
    const facts = Object.entries(memory.facts);
    if (facts.length > 0) {
        lines.push('## Facts', '');
        for (const [k, v] of facts)
...
// ─── A2A commands ──────────────────────────────────────────────────────────────
async function cmdA2a(input, hub) {
    const GR = '\x1b[90m', G = '\x1b[32m', Y = '\x1b[33m', R = '\x1b[0m';
    ...
    if (sub === 'send') {
        ...
        if (!id || !msg)
            return `${GR}Usage: /a2a send <agent-id> <message>${R}`;
        const res = await hub.sendMessage(id, msg, 'knowledge-share');
        if (!res.ok)
            return `${GR}Failed: ${res.error}${R}`;
    ...
    if (sub === 'ask') {
        ...
        if (!id || !q)
            return `${GR}Usage: /a2a ask <agent-id> <question>${R}`;
...
// ─── Terminal REPL ─────────────────────────────────────────────────────────────
async function terminalChat(llm, memory, awareness, systemPrompt, soulName, pluginLoader, a2a) {
    ...
    for await (const line of rl) {
        ...
        if (input === '/help') {
            console.log(cmdHelp(soulName));
            rl.prompt();
            continue;
        }
        if (input === '/clear') {
            console.log(`${GR}Context cleared.${R}`);
            rl.prompt();
            continue;
        }
        if (input === '/whoami') {
        ...
        if (input === '/export') {
            console.log(cmdExport(memory));
            rl.prompt();
            continue;
        }
        if (input.startsWith('/import ')) {
            console.log(cmdImport(memory, input.slice(8)));
            rl.prompt();
            continue;
        }
        // /plugins command
        ...
        if (input === '/memory' || input === '/memory list') {
            console.log(cmdMemoryList(memory));
            rl.prompt();
            continue;
        }
        if (input === '/memory clear') {
            console.log(cmdMemoryClear(memory));
            rl.prompt();
            continue;
        }
        ...
        if (input === '/git' || input === '/git log') {
            ...
            if (entries.length === 0) {
                console.log(`${GR}No git history.${R}`);
            }
            else {
                for (const e of entries)
                    console.log(`${GR}${e.hash}${R} ${GR}${e.date}${R} ${e.author}: ${e.msg}`);
            }
            rl.prompt();
            continue;
        }
        ...
        if (input.startsWith('/a2a')) {
            if (!a2a) {
                console.log(`${GR}A2A not enabled. Set COCAPN_A2A_SECRET or create cocapn/a2a-secret.json${R}`);
                rl.prompt();
                continue;
            }
        ...
        if (full)
            memory.addMessage('assistant', full);
        rl.prompt();
    }
}
// ─── Main ──────────────────────────────────────────────────────────────────────
async function main() {
    ...
    if (apiKey) {
        ...
        const ollama = await detectOllama();
        if (ollama && !llmConfig.model)
            llmConfig = { ...llmConfig, model: ollama.model };
    }
