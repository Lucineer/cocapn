...
export function reflect(memory, awareness) {
...
export function shouldReflect(memory) {
    const lastTs = memory.facts['_reflectionTs'];
    if (!lastTs)
        return memory.messages.length > 2;
...
function identifyPatterns(userMsgs, facts) {
    const patterns = [];
    if (userMsgs.length > 10)
        patterns.push('active conversation');
    if (facts.length > 5)
        patterns.push('accumulating knowledge');
    const questions = userMsgs.filter(m => m.content.includes('?')).length;
    if (questions > userMsgs.length * 0.4)
        patterns.push('curious interlocutor');
    return patterns;
}
